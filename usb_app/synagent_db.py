...
if getattr(sys, "frozen", False):
    ...
    BASE_DIR = Path(__file__).parent.parent.parent  # repo root fallback
...
<style>
  *{box-sizing:border-box;-webkit-user-select:none;user-select:none}
  ...
  body{background:#0d1117;color:#e2e8f0;font-family:system-ui,sans-serif;overflow-x:hidden;margin:0}
  ...
  #titlebar{-webkit-app-region:drag;height:36px;background:#080d14;display:flex;align-items:center;padding:0 12px;border-bottom:1px solid #1a2035;position:sticky;top:0;z-index:100}
...
<!-- Custom title bar -->
<div id="titlebar">
  <span class="text-lg mr-2" style="-webkit-app-region:no-drag">⚗️</span>
...
<!-- SEARCH -->
<div id="panel-search" class="panel active flex-1 p-4">
  <div class="relative mb-3">
    ...
    <input id="s-q" type="text" placeholder="Search keyword, SMILES, SMARTS, solvent, catalyst, reaction type..."
      ...
      style="background:#161b27;font-size:.9rem"
...
<div id="panel-browse" class="panel p-4">
  <div class="flex items-center gap-2 mb-3 flex-wrap">
    ...
    <select id="b-src" class="text-sm"><option value="all">All sources</option><option value="ord">ORD</option><option value="patent">Patent</option></select>
  ...
  <div class="card rounded-xl overflow-auto" style="max-height:calc(100vh - 200px)">
...
<div id="panel-stats" class="panel p-4">
  <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
    <div class="card rounded-xl p-4 text-center"><p class="text-3xl font-black text-white" id="d-total">—</p><p class="text-xs text-slate-500 mt-1 uppercase tracking-widest font-semibold">Total Reactions</p></div>
...
function card(r){
  ...
  const f=(label,val,maxLen=90)=>val?`<div class="mt-1.5"><span class="text-slate-500 text-xs">${label} · </span><span class="chip" onclick="event.stopPropagation();cp('${esc(val)}')" title="${esc(val)}">${val.slice(0,maxLen)}${val.length>maxLen?'…':''}</span></div>`:'';
...
async function doBrowse(page){
  ...
  const d=await fetch(`/api/search?q=${encodeURIComponent($('b-q').value.trim())}&source=${$('b-src').value}&sort=${$('b-sort').value}&page=${bPage}&page_size=${ps}`).then(r=>r.json());
  if(d.error)return;
...
async function openDetail(rid){
  const r=await fetch('/api/reaction/'+encodeURIComponent(rid)).then(x=>x.json());
  if(r.error)return;
...
async function loadStats(){
  ...
  $('d-details').innerHTML=[['High Confidence ≥0.6',s.high_confidence?.toLocaleString()],['Avg Yield',s.avg_yield!=null?s.avg_yield+'%':'—'],['Distinct Solvents',s.distinct_solvents?.toLocaleString()],['Distinct Catalysts',s.distinct_catalysts?.toLocaleString()]].map(([k,v])=>`<div class="flex justify-between text-xs"><span class="text-slate-500">${k}</span><span class="text-white font-semibold">${v}</span></div>`).join('');
  const tot=s.total||1;
  ...
  if(y?.values){const mx=Math.max(...y.values,1);$('yield-chart').innerHTML=y.values.map((v,i)=>`<div class="flex-1 flex flex-col items-center gap-0.5"><div class="yield-bar" style="height:${Math.round((v/mx)*66)+2}px" title="${y.labels[i]}: ${v}"></div><p class="text-xs" style="font-size:.5rem;color:#475569">${y.labels[i].split('-')[0]}</p></div>`).join('');}
}

function pages(page,total,topId,botId,fn){
  if(total<=1){[topId,botId].forEach(id=>{if($(id))$(id).innerHTML='';});return;}
  const h=`<button onclick="${fn}(${page-1})" ${page<=1?'disabled':''} class="px-2.5 py-1 rounded text-xs border border-[#21293d] text-slate-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed">←</button><span class="px-2 text-xs text-slate-400">${page}/${total}</span><button onclick="${fn}(${page+1})" ${page>=total?'disabled':''} class="px-2.5 py-1 rounded text-xs border border-[#21293d] text-slate-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed">→</button>`;
  [topId,botId].forEach(id=>{if($(id))$(id).innerHTML=h;});
}

function cp(t){navigator.clipboard.writeText(t).then(()=>{const x=$('toast');x.style.display='block';setTimeout(()=>x.style.display='none',1600);});}
...
# ── PySide6 native app window ────────────────────────────────────────────────
def run_server():
    uvicorn.run(api, host="127.0.0.1", port=8001, log_level="error")

def main():
    db = _find_db()
    print(f"DB: {db}" if db else "WARNING: DB not found")

    # Start FastAPI server in background thread
    t = threading.Thread(target=run_server, daemon=True)
    t.start()
    time.sleep(1.2)  # Let server come up

    # Launch the Qt native window
    ...
    app_qt.setApplicationName("SynAgent Reaction Database")
    app_qt.setOrganizationName("SynAgent")

    # Main window — no separate title bar, the HTML has one
    win = QMainWindow()
    win.setWindowTitle("SynAgent · Reaction Database")
    win.resize(1400, 900)
    win.setMinimumSize(900, 600)

    # Remove default window frame for cleaner look — keep native frame for resize
    # (frameless would remove resize handles, so keep frame)
    ...
    view.setUrl(QUrl("http://127.0.0.1:8001"))
